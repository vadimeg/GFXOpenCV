...
void correctnessChecker(const Mat &test_img, int imgNumber, bool normType) {
    string img_name = "test_img\\canny_" + std::to_string(imgNumber) + ((normType) ? ("_true") : ("_false")) + "_original.bmp";
    ...
    if (testStatus) 
        ...
        imwrite(string("canny_") + std::to_string(imgNumber) + ((normType) ? ("_true") : ("_false")) + "_testfailed.bmp", dst);
    }
}
...
int main() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        src = imread(imgpath[i], CV_LOAD_IMAGE_GRAYSCALE);
