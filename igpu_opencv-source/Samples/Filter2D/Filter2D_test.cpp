...
void correctnessChecker(const Mat &test_img, int imgNumber, int kernelSize) {
    string img_name = "test_img\\" + string("filter2D") + std::to_string(kernelSize) + "_" + std::to_string(imgNumber) + "_original.bmp";
    ...
    if (testStatus) 
        ...
        imwrite(string("filter2D") + std::to_string(kernelSize) + "_" + std::to_string(imgNumber) + "_testfailed.bmp", dst);
    }
}
...
int main() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < sizeof(kernelSize) / sizeof(int); j++) {
            ...
            measuredTime(&cv::filter2D, A, B, -1, kernel, cv::Point(-1, -1), 0, 4); 
